...
REQUIRED_COLUMNS = [
    "row_id",
    "error_flag",
    "error_types_all",  # Removed error_type_primary
    "error_confidence",
    "error_tags",
    "error_evidence",
]
...
def compute_metrics_from_output(output_csv: Path, ground_truth_csv: Path) -> Dict[str, Any]:
    ...
    if 'error_flag' not in df_out.columns or 'error_types_all' not in df_out.columns:
        return {
    ...
    def get_predicted_types(idx):
        pred_types = set()
        all_types = str(df_out.iloc[idx].get("error_types_all", ""))
