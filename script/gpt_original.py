...
def parse_ground_truth_label(label_str) -> set:
    """Parse ground truth label to extract error types from ERRORTYPE(details) format"""
    if pd.isna(label_str) or str(label_str).strip() == "":
        return set()
 
    error_types = set()
    
    # Find all occurrences of WORD( pattern
    matches = re.findall(r'([A-Z]+)\s*\(', str(label_str))
    ...
    for match in matches:
        canonical = CANONICAL_MAP.get(match.upper())
        if canonical:
            error_types.add(canonical)
...
If previous attempt failed, read the feedback and implement the specific fixes requested."""
    ...
    def execute(self, context: DetectionContext) -> DetectionContext:
        print(f"\n[STEP 1/3] Detection - {context.dataset_stem}")
...
def main() -> None:
    ...
    detector_prompt_path = "prompts/vanilla_examples.txt"
